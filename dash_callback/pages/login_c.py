...
app.clientside_callback(
    ...
    (fc_count,url) => {
        ...
        // 修改：始终显示验证码，不再根据失败次数判断
        return [{'display':'flex'}, {'height': 'max(45%,650px)'}, 1, title];
    }
    """,
    [
...
def login(
    ...
    # 修改：始终验证验证码，因为现在默认显示验证码
    if need_vc and (not vc_input or vc_input != pic_vc_value):
        return (
            dash.no_update,
...
@app.callback(
    ...
    [
        ...
        State('login-otp-verify-code-input', 'value'),
        State('login-otp-verify-code-pic', 'captcha'),
        State('login-url-location', 'href'),
    ],
    prevent_initial_call=True,
)
def otp_login(otp_value, user_name, verify_code_input, verify_code_pic, href):
    from otpauth import TOTP
    from database.sql_db.dao import dao_user
    from common.utilities.util_jwt import jwt_encode_save_access_to_session

    # 首先验证验证码
    if not verify_code_input or verify_code_input != verify_code_pic:
        return (
            fuc.FefferyFancyMessage(t__other('验证码错误，请重新输入'), type='error'),
            None,
        )
