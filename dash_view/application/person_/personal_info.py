...
from database.sql_db.dao.dao_user import UserInfo
...
def render_content(menu_access: MenuAccess, **kwargs):
    ...
    # 查询当前用户数据
    user_info = UserInfo.get_user_by_id(current_user_id)  # 假设你有这个方法

    score = user_info.score if user_info else 0
    level = user_info.user_level if user_info else '未知'
        ...
        fac.AntdCard(
            ...
            children=[
                fac.AntdDescriptions(
                    items=[
                        {'label': '当前等级', 'children': fac.AntdTag(content=level, color='blue')},
                        {'label': '当前积分',
                         'children': fac.AntdText(str(score), strong=True, style={'color': '#cf1322'})},
                        # ... 其他个人信息 ...
                    ],
                    bordered=True
                )
