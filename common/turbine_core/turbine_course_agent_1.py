...
{adaptive_focus}
"""),
            ("user", "请生成{count}道关于{chapter}的{type_name}，难度为{difficulty}。")
        ])
        try:
            ...
            return {
                **state,
                ...
                "error_message": "",  # 成功则清空错误
                "difficulty": difficulty,
                "retry_count": 0  # 重置计数
            }

        except Exception as e:
            logger.error(f"生成练习题JSON解析失败 (第 {retry_count + 1} 次): {e}")
            # 返回错误信息和增加的重试计数，让条件边决定是否重试
            return {
                **state,
                # 保留原始文本以便最后一次兜底显示
                "generated_questions": [{"raw_text": result if 'result' in locals() else "", "parse_error": str(e)}],
...
def create_agent_with_custom_model(
    ...
    Args:
        ...
        model_name: 模型名称
        custom_model_path: 自定义微调模型路径（如果有）
    """
    # 创建 LLM
    llm = ChatDeepSeek(
    ...
    # 如果提供了自定义模型路径，可以加载本地模型
    # 这里预留接口，用户可以替换为自己的微调模型
    custom_llm = None
    if custom_model_path:
        # 示例：加载本地微调模型
        # from transformers import AutoModelForCausalLM, AutoTokenizer
        # custom_llm = YourCustomLLMWrapper(custom_model_path)
        logger.info(f"加载自定义模型: {custom_model_path}")
        pass

    # 尝试导入并创建知识库
    knowledge_base = None
    try:
        # 这里假设用户已经有 MultiDocumentKnowledgeBase
        # 可以从用户的原始代码导入
        logger.info(f"知识库路径: {kb_path}")
        knowledge_base = MultiDocumentKnowledgeBase(kb_path, deepseek_api_key)
    except Exception as e:
        logger.warning(f"知识库初始化失败: {e}，将在无知识库模式下运行")
...
def main():
    ...
    print("\n功能说明:")
    print("1. 直接提问 - 回答透平机械相关问题")
    print("2. 生成练习题 - 例如：出5道轴流式压缩机的选择题")
    print("3. 批改作业 - 例如：批改：透平是将...的机械")
    print("4. 总结章节 - 例如：总结离心式压缩机")
    print("\n输入 'quit' 退出\n")
    ...
    while True:
        try:
            ...
            print(f"\n[意图: {result['intent']}, 置信度: {result['confidence']:.2f}]")
            print("-" * 60)
